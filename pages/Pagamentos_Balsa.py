...
if st.session_state.base_luck=='test_phoenix_joao_pessoa':
    ...
    if 'df_pag_final_forn' in st.session_state:
        ...
        df_pag_fornecedor = st.session_state.df_pag_final_forn.sort_values(by=['Data da Escala', 'Veiculo']).reset_index(drop=True)

        df_pag_fornecedor['Data da Escala'] = pd.to_datetime(df_pag_fornecedor['Data da Escala']).dt.strftime('%d/%m/%Y')

        total_a_pagar = df_pag_fornecedor['Valor Balsa'].sum()
        ...
        df_pag_fornecedor['Valor Balsa'] = df_pag_fornecedor['Valor Balsa'].apply(lambda x: format_currency(x, 'BRL', locale='pt_BR'))
